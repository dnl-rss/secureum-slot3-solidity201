...
The Solidity compiler generates **automatic getter functions** for all `public` state variables that **return the value** of the variable.

The **getter function** will `override` **external functions** in base contracts, if their parameter and return types match.

`public` state variables **cannot be overridden** themselves.
...
contract Base {
    function myString() virtual external returns (string memory) {
        return "my string";
    }
}

contract Derived is Base {    
    // overrides myString() in Base contract
    string public override myString = "another string";

    function test() public view returns (string memory) {        
        // returns "another string" because myString() from the Base contract was overridden
...
- Similar to `function` overriding
- Overriding modifier must be marked `override`
- Overridden modifier must be marked `virtual`
- Exception is that **modifiers are not overloaded**.
...
The `library` type is **more restricted** than `contract`:
- **no state variables**
- **cannot inherit nor be inherited**
- **cannot receive Ether**
- **cannot be destroyed**
- **must explicitly supply state variables of the calling contract** (has no way to name them otherwise)
- **functions can only be called directly if they do not modify the state** (without `DELEGATECALL`) because libraries are assumed to be stateless
