...
OpenZeppelin's smart contract libraries are the most commonly used libraries in smart contract projects.
...
`SafeERC20` wraps `ERC20` operations such that they throw an `Error`  and `revert` on failure when the token contract implementation returns `false`.
